...
def lowpass_filter(x, cutoff=10, fs=85, order=4):
    """
    x      : 1D array, 原始時序訊號
    cutoff : 截止頻率 (Hz)
    fs     : 取樣率 (Hz)
    ...
    # filtfilt 進行零相位濾波 (forward-backward)
...
def extract_features(data):
    ...
    if 1:
        ...
        gx, gy, gz = data[:, 3], data[:, 4], data[:, 5]
    if 0: # 0.8203
        ax, ay, az = lowpass_filter(ax), lowpass_filter(ay), lowpass_filter(az)
    ...
    features = {
        # Basic acceleration features # if cancal 0.82353485
...
def process_training_data(train_data_dir, train_info_path, mode_filter):
    ...
    for _, row in mode_info.iterrows():
        ...
        if os.path.exists(file_path):
            ...
            fold_list.append(row['hold racket handed'])

    # Convert to DataFrame
    X_train = pd.DataFrame(features_list)
    y_train = np.array(fold_list)
...
def get_model(model_name, random_state=42):
    ...
    if model_name == 'random_forest':
        ...
        return Pipeline([
            ('scaler', RobustScaler()),
            ('knn', KNeighborsClassifier(n_neighbors=10))  # n_neighbors 可調
        ])
...
def train_and_predict_for_mode(mode_filter, model_name, eval_mode=True, model_type='random_forest'):
    ...
    results_df = pd.DataFrame({
        'unique_id': file_ids,
        'hold racket handed': probabilities[:, 0]
    })
...
def combine_predictions(eval_mode=True, model_type='random_forest'):
    ...
    output_path = 'boss/submissions_final/holds_predictions.csv'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str,
                        choices=['random_forest', 'xgboost', 'lightgbm', 'catboost', 'stacking', 'svm', 'knn'],
    ...
    # python train_holders.py --model svm --mode all
    # 0.63479437
    # tbrain -> single -> baseline
    # svm ->
    #   filter 0.1~-0.1 0.63911102
    #   not filter, apply more xz skew 0.63903229
    #   filter 0.1~-0.1, apply 3to-3 swing xz skew 0.63909713
    #   not filter 0.63881228
    # random forest -> 0.63912723
...
"""
/opt/anaconda3/envs/aicup/bin/python /Users/charlie/MBP16/Master_Data/AICUP/PINGPONG/boss/code/boss_holder.py --model svm --mode all
"""